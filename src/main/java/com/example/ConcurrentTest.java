...
public class ConcurrentTest {
    ...
    public void Build() throws Exception {
     ...
     Set<Device> QueryRTree(String xmlData) throws Exception{
        ...
        // 根据传入的圆形和多边形构建并集
        List<Polygon> unionPolygons = polygonCircleUnion.getUnionPolygon(map.get("polygon"), map.get("circle"));
        ...
        List<Device> filteredDevices = new ArrayList<>(info.stream()
        ...
        // System.err.println("All Query time2: " + (end - start) + "ms");
         // 显式清理大集合，避免内存占用
         devicesInPolygon.clear();
...
//        list.add(xmlData3);
    ...
    @Benchmark
    public  Set<Device> testQueryRTree(Blackhole blackhole) throws Exception {
        // 执行查询
//        count = (count + 1) % 3;
//        return QueryRTree(list.get(count));
        Set<Device> resultSet = QueryRTree(list.get(0));
        blackhole.consume(resultSet); // 使用 Blackhole 吞噬结果，防止 JMH 优化
        return resultSet;
    }
